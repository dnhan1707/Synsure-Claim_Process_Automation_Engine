    ...
    routes = [
        {
            ...
            "description": "Upload files to a case with specified actions."
        },
        {
            "path": "/dupcheck", 
            "method": "post",
            "handler": check_duplicate_files,
            "description": "Check for duplicate files before uploading."
        }
    ]
    
    # Dynamically register routes
    for route_config in routes:
        getattr(router, route_config["method"])(route_config["path"])(route_config["handler"])
